...
function getOrAssignLocalAbVariant() {
  try {
    const stored = window.localStorage.getItem(AB_VARIANT_STORAGE_KEY);
    if (stored === "A" || stored === "B") return stored;
    const assigned = Math.random() < 0.5 ? "A" : "B";
    window.localStorage.setItem(AB_VARIANT_STORAGE_KEY, assigned);
    return assigned;
  } catch (error) {
    return null;
  }
...
function getAbTelemetrySnapshot() {
  try {
    const variant = typeof window.getAbVariant === "function"
      ? window.getAbVariant()
      : getOrAssignLocalAbVariant();
...
async function createNewSession() {
  ...
  try {
    ...
    const sessionPayload = {
      ...
      player_id: playerId,
      session_id: sessionId,
      ab_variant: typeof window.getAbVariant === "function"
        ? window.getAbVariant()
        : getOrAssignLocalAbVariant(),
      game_version: gameVersion,
      release_channel: releaseChannel,
